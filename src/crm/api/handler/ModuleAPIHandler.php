...
class ModuleAPIHandler extends APIHandler
{
    ...
    public function getCustomView($customViewId)
    {
        try {
            ...
            $responseInstance->setData(MetaDataAPIHandler::getInstance()->getZCRMCustomView($this->module->getAPIName(),
                $responseJSON['custom_views'][0], $categories));
     ...
     */
    public function getAllCustomViews($param_map): \zcrmsdk\crm\api\response\BulkAPIResponse
    {
        try {
            ...
            foreach ($customViews as $customView) {
                $customViewInstances[] = MetaDataAPIHandler::getInstance()->getZCRMCustomView(
                    $this->module->getAPIName(), $customView, $categories
                );
            }
     ...
     */
    public function updateModuleSettings(): \zcrmsdk\crm\api\response\APIResponse
    {
        try {
     ...
     * Returns api response
     */
    public function updateCustomView($customViewInstance): \zcrmsdk\crm\api\response\APIResponse
    {
        try {
     ...
     */
    public function getAllRelatedLists(): \zcrmsdk\crm\api\response\BulkAPIResponse
    {
        try {
     ...
     */
    public function getRelatedListDetails($relatedListId): \zcrmsdk\crm\api\response\APIResponse
    {
        try {
    ...
    public function getLayouts($allLayoutDetails): array
    {
        $layoutArray = array();
    ...
    {
        $fieldInstance = ZCRMField::getInstance($fieldDetails['api_name']);
        $fieldInstance->setSequenceNumber(isset($fieldDetails['sequence_number']) ? $fieldDetails['sequence_number']
                                                                                    + 0 : 0);
        $fieldInstance->setId($fieldDetails['id']);
        $fieldInstance->setMandatory(isset($fieldDetails['required']) && (boolean) $fieldDetails['required']);
        ...
        if (array_key_exists("view_type", $fieldDetails)) {
            ...
            if ($viewTypeArray['view']) {
                $fieldLayoutPermissions[] = "VIEW";
            }
            if ($viewTypeArray['quick_create']) {
                $fieldLayoutPermissions[] = "QUICK_CREATE";
            }
            if ($viewTypeArray['create']) {
                $fieldLayoutPermissions[] = "CREATE";
            }
            if ($viewTypeArray['edit']) {
                $fieldLayoutPermissions[] = "EDIT";
            }
        ...
        if (sizeof($pickListArray) > 0) {
            $pickListInstanceArray = array();
            foreach ($pickListArray as $pickList) {
                $pickListInstanceArray[] = self::getPickListValueInstance($pickList);
            }
            $fieldInstance->setPickListFieldValues($pickListInstanceArray);
        }
        ...
        if (array_key_exists("currency", $fieldDetails) && sizeof($fieldDetails['currency']) > 0) {
            ...
            $fieldInstance->setPrecision(isset($currencyFieldDetails['precision']) ? $currencyFieldDetails['precision']
                                                                                     + 0 : 0);
        ...
        if (array_key_exists("auto_number", $fieldDetails) && sizeof($fieldDetails['auto_number']) > 0) {
            ...
            $fieldInstance->setPrefix(array_key_exists('prefix',
                $fieldDetails['auto_number']) ? $fieldDetails['auto_number']['prefix'] : null);
            $fieldInstance->setSuffix(array_key_exists('suffix',
                $fieldDetails['auto_number']) ? $fieldDetails['auto_number']['suffix'] : null);
            $fieldInstance->setStartNumber(array_key_exists('start_number',
                $fieldDetails['auto_number']) ? $fieldDetails['auto_number']['start_number'] + 0 : null);
        }
    ...
    {
        $lookupInstance =
            ZCRMLookupField::getInstance($lookupDetails['api_name'] ?? null);
    ...
    public function getZCRMLayout($layoutDetails)
    {
        ...
        if ($layoutDetails['created_by'] != null) {
            $userInstance =
                ZCRMUser::getInstance((($layoutDetails['created_by']['id'])), $layoutDetails['created_by']['name']);
            $layoutInstance->setCreatedBy($userInstance);
        }
        if ($layoutDetails['modified_by'] != null) {
            $userInstance =
                ZCRMUser::getInstance((($layoutDetails['modified_by']['id'])), $layoutDetails['modified_by']['name']);
        ...
        foreach ($accessibleProfileArray as $profile) {
            ...
            $accessibleProfileInstances[] = $profileInstance;
        }
                ...
                if (isset($layoutDetails['convert_mapping'][$convertModule])) {
                    ...
                    if (isset($contactsMap['fields'])) {
                        ...
                        foreach ($fields as $field) {
                            $convertMappingFieldIns =
                                ZCRMLeadConvertMappingField::getInstance($field['api_name'], $field['id']);
